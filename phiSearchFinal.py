    ...
    def socialWorker(self):
        ...
        try:
                with open(self.record, 'r+') as file:
                    ...
                    if SocialWorker:
                        token = "*SocialWorker*"
                        self.retrievedWords["*SocialWorker*"] = SocialWorker
                        updated_lines = []
                        for line in lines:
    ...
    def fax(self):
        ...
        try:
            with open(self.record, 'r+') as file:
                lines = file.readlines()
                updated_lines = []
                fax_pattern = r'(\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4})'
                fax_index = 0

                for line in lines:
                    if "fax" in line.lower():
                        matches = re.findall(fax_pattern, line)
                        for match in matches:
                            token = f"*fax{fax_index}*"
                            self.retrievedWords[token] = match
                ...
                for line in lines:
                    ...
                    if cnum in line:
                        if certificate:
                            ...
                            self.retrievedWords[token] = certificate
                            certificateCount += 1
                            line = line.replace(certificate, token)
                    updated_lines.append(line)
                file.seek(0)
                file.truncate(0)
                file.writelines(updated_lines)
            ...
            with open(self.record, 'r+') as file:
                ...
                for line in lines:
                    ...
                    for url in urls:
                        urlCount += 1
                        token = "*URL*"
                        self.retrievedWords["*URL*"] = url
                        line = line.replace(url, token)
                    updated_lines.append(line)
                file.seek(0)
                file.truncate(0)
                file.writelines(updated_lines)
...
def main():
    ...
    if "deid_complete" not in st.session_state:
        st.session_state.deid_complete = False
    ...
    if st.button('Enter'):
        ...
        st.session_state.deid_complete = True
        st.session_state.output_file = outputFile
        st.session_state.token_file = retrievedWordsFile

        print("PHI Final Count: ", found)
        print(f"Token map text file saved as: {retrievedWordsFile}")
        print(f"De-identified output file saved as: {outputFile}")
        

    if st.session_state.deid_complete:
        with open(st.session_state.token_file, "r") as f:
            st.download_button("Download Token Map Text File", f, file_name=st.session_state.token_file)

        with open(st.session_state.output_file, "r") as f:
            st.download_button("Download De-identified Output File", f, file_name=st.session_state.output_file)

    st.subheader("Re-identification Program")
    name3 = st.text_input("Enter text file")
    name4 = st.text_input("Enter output file")

    if "reid_complete" not in st.session_state:
        st.session_state.reid_complete = False
    ...
    if(st.button('Submit')):
        ...
        print(f"Re-identification complete. Output written to {reidentifiedFile}")

        st.session_state.reid_complete = True
        st.session_state.reid_file = reidentifiedFile

    if st.session_state.get("reid_complete", False):
        reid_file = st.session_state.get("reid_file")
        if reid_file:
            with open(reid_file, "r") as f:
                st.download_button("Download Re-identified File", f, file_name=reid_file)
...
# streamlit run phiSearchFinal.py